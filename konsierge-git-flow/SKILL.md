...
description: "Use for every Git delivery task in a Konsierge repository, including checkout selection, optional worktree isolation, task commits, target integration, pushes, and conditional deployment verification."
---
...
- Choose the implementation checkout using the criteria below. In the main checkout, commit directly on the persistent task branch. When isolation is needed, use a linked worktree on a temporary descriptive branch such as `KON-1234-authorization-implementation`, then cherry-pick its commits in order into the persistent task branch. Never merge the temporary branch into the task branch.
...
## Choose the Implementation Checkout

Prefer the main checkout for small, straightforward, low-risk changes touching few files when the task branch is available and no concurrent writer or overlapping change would interfere. Unrelated dirty files alone do not require isolation if they can be preserved and excluded from staging and no branch switch is needed.

Use a separate worktree for substantial or risky changes, conflicting concurrent work, or when the main checkout's branch or working state cannot safely host the task. Reuse a suitable existing task worktree after checking its ownership, branch, and status. Do not create a fresh worktree for every edit or implementation step. Decide routine checkout selection without asking for permission; respect an explicitly requested checkout.
...
4. Select the implementation checkout using the criteria above. For work in the main checkout, use the persistent task branch and skip new-worktree setup. If isolation is needed and no suitable task worktree exists, create one from the current persistent task branch tip under `~/dev/konsierge/worktrees`, for example `~/dev/konsierge/worktrees/travelmart-KON-1234-authorization-implementation`. Its temporary branch must match `KON-[0-9]+-[a-z0-9]+(?:-[a-z0-9]+)*` and describe the work clearly. Do not use detached HEAD.
5. Only for a new worktree, from the main repository root, copy its local `.env` before setup or tests: `cp .env <worktree-path>/.env`. Do not create a placeholder when the main checkout has no `.env`, and never commit the copied file.
6. Only for a new worktree, if `.gitmodules` declares the `contracts` submodule, initialize it in the new worktree with `git submodule update --init --recursive contracts`, then run `bin/contracts-update` from the worktree root before running tests. This setup does not make `contracts` part of the task scope; do not stage or commit it unless the user explicitly requested contract changes.

## Implement and Transfer Commits

### Main checkout

Implement and run proportionate checks on the persistent task branch. Stage only task changes and create atomic commits using [`konsierge-commit-conventions`](../konsierge-commit-conventions/SKILL.md). Verify the task diff and commit SHAs, preserving unrelated changes. No cherry-pick or implementation-worktree cleanup is needed. Continue to integration only when requested and the checkout is ready for branch switching.

### Isolated worktree

1. Implement and verify the requested work in the selected implementation worktree.
...
3. Cherry-pick only task changes missing from dev, resolve their conflicts, and verify the result. Do not merge `origin/dev` or another branch into `KON-11419-dev`. Conflict-specific edits belong here; choose isolation for additional coding using the same checkout criteria above.
...
Enter this section only when the user explicitly requests deployment monitoring or both `dev` and `master` are being pushed. Otherwise finish after remote SHA and containment verification; cleanup of any session-owned implementation worktree must already be complete.
...
This section applies only when a session-owned implementation worktree was used. Cleanup is part of commit transfer. Perform it immediately after cherry-picks and task-branch checks succeed, before merging, pushing, or monitoring deployment. Run a final worktree audit on every exit path; if unique work prevents removal, report the exact path, size, and reason instead of silently retaining it.
...
Report the persistent task branch, task commit SHAs (and source-to-task mapping when transferred), merge commits for existing targets, remote SHAs, checks, pushes, and final worktree state. When monitoring was required, also report each monitored pipeline/job or deployment ID, URL, and final status.
