...
description: "Apply the Konsierge branch, commit, target-branch push, and conditional deployment-verification workflow. Use for every Git delivery task in a Konsierge repository: work and commit in a `KON-XXXX` branch, default to `KON-0000` when no ticket is specified, deliver to the available `dev` and/or `master` targets, and never push a feature branch."
---
...
Fetch `origin` again immediately before merging. First check which delivery targets exist and whether the matching `KON-XXXX-dev` branch exists locally or on `origin`. Apply the steps below only to existing target branches; skip every step for an absent target.
