...
- For architecture, several independent components, or work that cannot fit safely in one context, record a concrete plan, isolate the work when useful, and delegate only independent, bounded streams. Reuse prior decisions and authorization; seek approval only for unresolved material choices or actions beyond the authorized scope. Continue independent, authorized work while required answers are pending.
- Use [brainstorming](../brainstorming/SKILL.md) for unresolved design choices and [interview-me](../interview-me/SKILL.md) for unclear intent. Group independent questions; wait for prerequisite answers before dependent ones. Clear, authorized work needs no additional approval round.
- For bugs with an uncertain cause or a failed fix, use [hypothesis-driven-debugging](../hypothesis-driven-debugging/SKILL.md) before further implementation.
